...
复杂任务走本地 Codex 自动队列并回推进度；聚合 RMIT 校园邮件 / 个人 Gmail / 游戏新闻；
...
```
总工出阶段规格(带验收标准) → Codex 在分支上实现 + 跑验证 → 总工 diff 级评审 → 合并 → 下一阶段
```

- **阶段单位**：一次约 5 小时 token 额度 = 一个阶段。
- **每阶段交付物**：`docs/phase-N-*.md`（规格）+ 对应分支的实现 + 全绿的 `npm test` / `npm run lint`。
- **Codex 不得**：改动 `.env`/凭据/`data/`；删除既有 dead code（只标记）；做规格外的"顺手优化"。
